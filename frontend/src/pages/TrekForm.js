...
function TrekForm() {
  ...
  const validateForm = () => {
    ...
    if (!formData.basePrice || formData.basePrice < 0) errors.basePrice = 'Base price must be a non-negative number';
    if (!formData.imageUrl.trim()) errors.imageUrl = 'Image URL is required';
    batches.forEach((batch, index) => {
       if (!batch.startDate) errors[`batch_${index}_startDate`] = 'Start date is required';
       if (!batch.endDate) errors[`batch_${index}_endDate`] = 'End date is required';
       if (!batch.price || batch.price < 0) errors[`batch_${index}_price`] = 'Price must be non-negative';
       if (!batch.maxParticipants || batch.maxParticipants <= 0) errors[`batch_${index}_maxParticipants`] = 'Max participants must be positive';
    });
  ...
  const handleSubmit = async (e) => {
    ...
    const trekPayload = {
      ...formData,
      batches: batches.map(b => ({ ...b, price: Number(b.price), maxParticipants: Number(b.maxParticipants) })),
      itinerary: itinerary.map(day => ({ ...day, activities: day.activities.filter(act => act.trim() !== '') })),
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <FormSection title="Basic Information">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <InputField label="Primary Image URL" id="imageUrl" value={formData.imageUrl} onChange={handleInputChange} required error={formErrors.imageUrl} placeholder="https://..." />
            <div className="mb-4">
            ...
            {batches.map((batch, index) => (
              ...
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                  <InputField label={`Start Date`} id={`batch_startDate_${index}`} type="date" value={batch.startDate} onChange={(e) => handleBatchChange(index, 'startDate', e.target.value)} required error={formErrors[`batch_${index}_startDate`]} />
                  <InputField label={`End Date`} id={`batch_endDate_${index}`} type="date" value={batch.endDate} onChange={(e) => handleBatchChange(index, 'endDate', e.target.value)} required error={formErrors[`batch_${index}_endDate`]} />
                  <InputField label={`Price (INR)`} id={`batch_price_${index}`} type="number" min="0" step="0.01" value={batch.price} onChange={(e) => handleBatchChange(index, 'price', e.target.value)} required error={formErrors[`batch_${index}_price`]} />
                  <InputField label={`Max Participants`} id={`batch_maxParticipants_${index}`} type="number" min="1" value={batch.maxParticipants} onChange={(e) => handleBatchChange(index, 'maxParticipants', e.target.value)} required error={formErrors[`batch_${index}_maxParticipants`]} />
                </div>
                {batches.length > 1 && (
                  <Button
                    variant="icon"
